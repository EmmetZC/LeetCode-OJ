...
#define INFI 1000000u

class Solution {
private:
	unsigned getDistance(unsigned a, unsigned b)
	{
		if (a == b) {
			return 0;
		}
		...
		string strA = words[a], strB = words[b];
	...
	void initDistances() {
		distances.assign(words.size(), vector<unsigned>(words.size(), 0));
		neighbors.assign(words.size(), unordered_set<unsigned>());
	...
	void init(string start, string end, unordered_set<string> &dict) {
		cout << "init() started" << endl;
		words.assign(dict.cbegin(), dict.cend());
		words.push_back(end);
	...
	void newLadder(vector<unsigned> ladder) {
		if (ladder.size() == ladderLen) {
			ladders.push_back(ladder);
		} else if (ladder.size() < ladderLen) {
			ladders.clear();
			ladders.push_back(ladder);
			ladderLen = ladder.size();
			cout << "Ladder Len: " << ladderLen << endl;
		}
	}

	vector<string> words;
	vector<vector<unsigned>> distances;
	vector<unordered_set<unsigned>> neighbors;
	vector<vector<unsigned>> ladders;
	unsigned START, END;	// id of start and end
	unsigned ladderLen = INFI;

	void backtrack(vector<unsigned> partialLadder, vector<unsigned> left) {
		unsigned last = partialLadder.back();
		if (distances[last][END] + partialLadder.size() > ladderLen) {		// Pruning
			return;
		} 
		if (distances[last][END] == 0) {
			newLadder(partialLadder);
		} else if (distances[last][END] == 1) {
			partialLadder.push_back(END);
			newLadder(partialLadder);
		} else {
			for (auto iter = left.begin(); iter < left.end(); ++iter) {
				auto chosen = *iter;
				if (neighbors[last].count(chosen) >0) {
					iter = left.erase(iter);
					partialLadder.push_back(chosen);
					backtrack(partialLadder, left);
					partialLadder.pop_back();
					iter = left.insert(iter, chosen);
				}
			}
		}
	}
...
public:
	vector<vector<string>> findLadders(string start, string end, unordered_set<string> &dict) {
		init(start, end, dict);
		vector<unsigned> initLadder = { START };
		vector<unsigned> left;
		for (unsigned i = 0; i < words.size() - 1; ++i) {
			left.push_back(i);
		}
		backtrack(initLadder, left);
		cout << ladders.size() << "\t" << ladderLen << endl;
